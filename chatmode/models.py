...
from sqlalchemy import (
    Column, String, Text, Boolean, Float, Integer, DateTime, ForeignKey,
    JSON, Index, UniqueConstraint, CheckConstraint, Enum as SQLEnum
)
...
class Agent(Base):
    ...
    voice_settings = relationship("AgentVoiceSettings", back_populates="agent", uselist=False, cascade="all, delete-orphan")
    memory_settings = relationship("AgentMemorySettings", back_populates="agent", uselist=False, cascade="all, delete-orphan")
    permissions = relationship("AgentPermissions", back_populates="agent", uselist=False, cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        CheckConstraint('max_tokens >= 1 AND max_tokens <= 128000', name='check_max_tokens'),
...
class AgentVoiceSettings(Base):
    ...
    __table_args__ = (
        CheckConstraint('speaking_rate >= 0.5 AND speaking_rate <= 2.0', name='check_speaking_rate'),
...
class AgentMemorySettings(Base):
    ...
    __table_args__ = (
        CheckConstraint('retention_days >= 1 AND retention_days <= 365', name='check_retention'),
...
class Message(Base):
    ...
    sender_id = Column(String(36), ForeignKey("agents.id"), nullable=True)  # NULL for admin/human
    ...
    voice_asset = relationship("VoiceAsset", back_populates="message", uselist=False, cascade="all, delete-orphan")
