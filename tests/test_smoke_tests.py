...
class TestProfileLoading:
    ...
    def test_profile_with_extra_prompt(self):
        ...
        try:
            ...
            assert (
                agent.system_prompt.count("Additional instructions for testing.") == 1
            )
    ...
    def test_profile_backward_compatibility_without_optional_fields(self):
        ...
        try:
            settings = load_settings()
            agent = ChatAgent(
                name="legacy", config_file=profile_path, settings=settings
            )
...
class TestSoloMode:
    ...
    def test_admin_agent_creates_clarifying_questions(self):
        ...
        with patch.object(admin.provider, "chat") as mock_chat:
            ...
            assert (
                response == f"Let's discuss: {topic}. What are your initial thoughts?"
            )

    @pytest.mark.asyncio
    async def test_session_supports_solo_mode(self):
        ...
        with patch("chatmode.session.load_agents") as mock_load:
            ...
            success = await session.start("Test topic")
    ...
    def test_memory_retrieval_respects_filters(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results = memory.query(
                "test query", k=5, session_id="session_123", agent_id="agent_1"
            )
